...
const logCyan = (log) => console.log('\x1b[36m%s\x1b[89m', log);
const logBlue = (log) => console.log('\x1b[34m%s\x1b[89m', log);
...
// watch file
logYellow('listening for changes');
let wait = false;
fs.watch('./test/app.fy', () => {
    if (wait) return;
    wait = !! setTimeout(() => {
        // logger
        const date = new Date();
        logCyan(`file updated ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`);
